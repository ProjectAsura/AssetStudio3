...
// Constant Values.
//-----------------------------------------------------------------------------
#include "../res/shaders/Compiled/Imgui_VS.inc"
#include "../res/shaders/Compiled/Imgui_PS.inc"
#include "../res/shaders/Compiled/ImguiCube_PS.inc"
...
)
{
    ...
    // 頂点シェーダ
    {
        auto hr = m_pDevice->CreateVertexShader( Imgui_VS, sizeof(Imgui_VS), nullptr, m_pVS.GetAddress() );
        if (FAILED(hr))
        { return false; }
        ...
        hr = m_pDevice->CreateInputLayout( elements, 3, Imgui_VS, sizeof(Imgui_VS), m_pIL.GetAddress() );
        if (FAILED(hr))
        { return false; }
    }
    ...
    // ピクセルシェーダ.
    {
        auto hr = m_pDevice->CreatePixelShader( Imgui_PS, sizeof(Imgui_PS), nullptr, m_pPS.GetAddress() );
        if (FAILED(hr))
        { return false; }
    }
    ...
    // キューブマップ用ピクセルシェーダ.
    {
        auto hr = m_pDevice->CreatePixelShader(ImguiCube_PS, sizeof(ImguiCube_PS), nullptr, m_pPSCube.GetAddress());
        if (FAILED(hr))
        { return false; }
    }
