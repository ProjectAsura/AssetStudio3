...
#include "../res/shaders/Compiled/SkyBox_VS.inc"
#include "../res/shaders/Compiled/SkyBox_PS.inc"
#include "../res/shaders/Compiled/SkySphere_VS.inc"
#include "../res/shaders/Compiled/SkySphere_PS.inc"
#include "../res/shaders/Compiled/SkySphereFlow_PS.inc"
...
bool SkyBox::Init(ID3D11Device* pDevice)
{
    ...
    // 頂点シェーダ・入力レイアウトの生成.
    {
        hr = pDevice->CreateVertexShader( SkyBox_VS, sizeof(SkyBox_VS), nullptr, m_pVS.GetAddress() );
        if (FAILED(hr))
        {
        ...
        hr = pDevice->CreateInputLayout( elements, 1, SkyBox_VS, sizeof(SkyBox_VS), m_pIL.GetAddress() );
        if (FAILED(hr))
        {
    ...
    // ピクセルシェーダの生成.
    {
        hr = pDevice->CreatePixelShader( SkyBox_PS, sizeof(SkyBox_PS), nullptr, m_pPS.GetAddress() );
        if (FAILED(hr))
        {
...
bool SkySphere::Init(ID3D11Device* pDevice, int tessellation)
{
    ...
    // 頂点シェーダ・入力レイアウトの生成.
    {
        hr = pDevice->CreateVertexShader(SkySphere_VS, sizeof(SkySphere_VS), nullptr, m_pVS.GetAddress());
        if (FAILED(hr))
        {
            ELOG("Error : ID3D11Device::CreateVertexShader() Failed.");
            return false;
        ...
        hr = pDevice->CreateInputLayout(elements, 4, SkySphere_VS, sizeof(SkySphere_VS), m_pIL.GetAddress());
        if (FAILED(hr))
        {
            ELOG("Error : ID3D11Device::CreateInputLayout() Failed.");
            return false;
    ...
    // ピクセルシェーダの生成.
    {
        hr = pDevice->CreatePixelShader(SkySphere_PS, sizeof(SkySphere_PS), nullptr, m_pPS.GetAddress());
        if (FAILED(hr))
        {
            ELOG("Error : ID3D11Device::CreatePixelShader() Failed.");
            return false;
    ...
    // フロー用ピクセルシェーダの生成.
    {
        hr = pDevice->CreatePixelShader(SkySphereFlow_PS, sizeof(SkySphereFlow_PS), nullptr, m_pPSFlow.GetAddress());
        if (FAILED(hr))
        {
            ELOG("Error : ID3D11Device::CreatePixelShader() Failed.");
            return false;
